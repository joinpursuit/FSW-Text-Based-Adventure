...
const catMadLibs = () => {
  ...
  They wash themselves by licking their fur with their scratchy ` + chalk.green(`${bodyPart} `) + `.
  Cats purr. It's ` + chalk.green(`${adverb} `) + `a soothing sound that can even win the heart of a non-cat lover.
  Finally, a cat is reputed to have ` + chalk.green(`${number} `) + `lives, which makes it the cat's meow!`);
};
...
const dogMadLibs = () => {
  ...
  Just rub a dog's ` + chalk.green(`${bodyPart} `) + `and you will have a pal for life.
...
const aussieMadLibs = () => {
  ...
  If you are a nature lover, you must put this exotic land Australia as number` + chalk.green(` ${number} `) + `on your places-to-go list!`);
};
...
You nod slowly, in shocked at how real everything looks.`));
  ...
  if (verb === - 1) {
    ...
    console.log(`You ` + chalk.green(`${threeChoices[verb]}`) + `, then you see a small wooden house a short distance away.\nYou approach the front door.`);
  }
  /////////////////////// woodenHouse /////////////////////////////////
  ...
  if (woodenHouse === `go in`) {
    ...
    if (woodenHouseExt === "sit" || woodenHouseExt === "stand") {
      ...
      console.log(`You ask what's for dinner?\nThe ` + chalk.green(`${threeChoices[adjective]}`) + ` old man replies,\n`
      ...
      + chalk.cyan(`I got another one! Darn `) + chalk.green(`${paths[path]}s`)
