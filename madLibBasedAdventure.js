...
console.log(
  chalk.blue(`You will be one of the first to test out Wacky Mad Lib VR Adventure. At the end, you will receive a personalized postcard for participating in the trial run. Throughout your adventure you will be confronted by different situations. These events will personalize your experience.`)); // instructions
...
///////////////////////////////  Game Loop  ///////////////////////////////////////
const gameLoop = () => {
  console.log(`You put on the VR goggles and earbuds. Instantly, you are standing in the woods. You hear Maggie in your ear,\n` + chalk.blue(`Hey ` + chalk.green(`${name}`)) + chalk.blue(`, can you hear me ok? You nod slowly, in shocked at how real everything looks.`));

  readline.keyInPause();
  console.clear();

  if (
    readline.keyInYNStrict(`She ask you again,\n` + chalk.blue(`Hey ` + chalk.green(`${name}`)) + chalk.blue(`, can you hear me ok?`))) {
    console.clear();
    console.log(chalk.blue(`\nGreat!`) + `, Maggie responds.`);
  } else {
    console.clear();
    console.log(
      chalk.blue(`So how come you’re answering me? He, He, He. Just kidding`) + `, she snickers.`);
  }
  ...
  let pathIdx = readline.keyInSelect(paths, `Choose one:`, {
  ...
  if (pathIdx === -1) {
    ...
    console.log(`Apprehensive at the idea, you choose the path with the sign marked ` + chalk.green(`${paths[pathIdx]}`) + `.\nYou think to yourself, this VR experience would be the best time to get over your fears of ` + chalk.green(`${paths[pathIdx]}s`) + `.\nHeistately, you start walking along the path. After sometime, You hear rustling in the bushes around you.`);
  }
  ...
  let threeAdjectives = randomizeArray(adjectives);
  adjective = readline.keyInSelect(threeAdjectives, `choose an adjective`, {
    cancel: `choose for me`,
  });
  console.clear()

  if (adjective === -1) {
    adjective = threeAdjectives[Math.floor(Math.random() * threeAdjectives.length)];
    ...
    console.log(`You see a/an ` + chalk.green(`${threeAdjectives[adjective]} `) + `old man at a large black\npot stirring something with a large wooden spoon.\nHe turns to you and says...`);

    let threeExclamation = randomizeArray(exclamations);
    
    let exclamationIdx = readline.keyInSelect(threeExclamation, `choose an exclamatory word`,{cancel: `choose for me`});
    
    console.clear()

    if (exclamationIdx === -1) {
      exclamation = threeExclamation[Math.floor(Math.random() * threeExclamation.length)];
      
      console.log( chalk.green(`${exclamation}!`) + chalk.cyan(` You're just in time for dinner. Have a seat.`));
    } else {
      
      exclamation = threeExclamation[exclamationIdx]
      console.log(chalk.green(`${exclamation}!`) + chalk.cyan(` You're just in time for dinner. Have a seat.`));
    }
    ...
    if (woodenHouseExt === "sit" || woodenHouseExt === "stand") {
      console.log(`As you ` + chalk.green(`${woodenHouseExt}`) + ` watching the ` + chalk.green(`${threeAdjectives[adjective]}`) + ` old man stir the pot, you see a/an`);
  ...
  let threeAdverbs = randomizeArray(adverbs);
  let adverbIdx = readline.keyInSelect(threeAdverbs, `Which do you choose?`, {
    cancel: `choose for me`,
  });

  console.clear()
  if (adverb === -1) {
    adverb = threeAdverbs[Math.floor(Math.random() * threeAdverbs.length)];
    ...
    adverb = threeAdverbs[adverbIdx]
    console.log(`The rabbit ` + chalk.green(`${threeAdverbs[adverbIdx]}`) + ` hops down the path.\nCurious. You follow it.\nThe rabbit starts moving faster and you lose sight of it.`);
  }
  ...
  if (readline.keyInYNStrict(`Do you join?`)) {
    ...
    console.log("you hear a loud " +  chalk.red("Bang!") + "\nThe guys deburst.");
    readline.keyInPause();
    console.clear();
